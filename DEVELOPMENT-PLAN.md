...
当前进度（2026-07-25）：自动前台切换、系统主题、正式托盘图标和开机启动均已真机通过；启用后可随系统启动，取消后重启不再启动。Orb 在固定/自动模式下的半隐藏 hover、移开回藏、边缘/自由拖放、Capsule 收起及退出重开位置恢复均已由用户确认。双屏、侧边任务栏、显示器断开与混合 DPI 仍待对应环境复验。
...
当前进度（2026-07-25）：settings schema v3 的 v1/v2 迁移、四形态位置恢复/回退/clamp 与仓库写盘已自动化验证；中英文核心流程及真实退出重开后的位置恢复已真机通过。共享玻璃材质与 token 级全量对比度审计已完成：小字号语义色在透明玻璃黑/白极端背景和 aurora 组合下 ≥4.5:1，accent 焦点/图形 ≥3:1；四形态 Light/Dark 固定视口截图通过。
...
| Light/Dark/Auto | 完整         | 三态持久化真机通过；四形态 Light/Dark 材质截图与对比度审计通过   | 完整                        |
| 自动前台切换    | 完整         | D-3 完成（真机验收通过）                                         | 完整                        |
| DPI/IDE 贴附    | 完整         | 部分（多屏/混合 DPI 真机待验）                                   | 完整                        |
| 拖动/贴边/展开  | 完整         | H 切片 3 四态交互、自由位置及重启恢复真机通过；多屏待验          | 完整                        |
| 托盘            | 完整         | E-F 真机通过；正式九档 ICO 已打包并完成视觉复验                  | 完整                        |
