...
- 拖动、贴边、悬停露出、点击展开、键盘操作、托盘菜单、开机启动。
- 用户偏好持久化。
- MCP 只读工具和现有数据核心。
...
- 拖动结束时按最终位置分流：碰到或越过 workArea 左右边缘才吸附为 revealed placement，移开 1 秒回半隐藏；未碰边则完整保留在松手位置并保存为自由 placement。真实物理外边缘半隐藏时只保留 24 DIP 可见；内部接缝或侧边任务栏处保持完整可见并留 6 DIP 边距。
- hover 半隐藏 Orb 时只露出同边完整 Orb，使整个 82×136 命中区可点击、可拖动；hover 不直接展开 Capsule。
- 完整贴边 Orb 离开满 1 秒后自动回半隐藏，窗口外点击立即回半隐藏；自由 Orb 离开或窗口外点击均保持原位。
...
- 点击完整 Orb 后展开；点击窗口外空白处立即收起，或离开整个 Capsule 满 1 秒后收起，过渡 180ms ease-out。收起恢复展开前的 Orb placement：自由位置回 Orb，边缘位置回半隐藏。
...
- 收起、展开、悬停延迟、拖动阈值、360 式半隐藏贴边、多显示器。

Gate：四形态均使用真实数据；边缘 Orb 只露出 24 DIP，hover 露出完整 Orb，移开 1 秒重新半隐藏，点击才展开 Capsule；Orb 拖动碰边时自动吸附并进入回藏流程，未碰边时可在任意位置原位停留。Capsule 离开 1 秒自动收起，窗口外点击立即收起；收起按展开前 placement 回自由 Orb 或边缘半隐藏态。固定 Orb 模式与自动模式行为一致，连续操作无闪白、误触或位置跳变。

### Phase 6 — Windows 系统集成

- 前台进程识别、自动模式、DPI、系统主题、托盘、开机启动。

当前进度（2026-07-24）：自动前台切换、系统主题与托盘已真机通过；开机启动与 per-surface 位置/显示器/贴边持久化的代码和 portable 打包验证完成。Orb 交互已按真机反馈修为：固定/自动模式都启用控制器，半隐藏 hover 只露出 Orb，完整贴边 Orb 移开 1 秒重新半隐藏，拖动可原位自由悬浮；Capsule 离开 1 秒自动收起并恢复展开前 placement。开机启动及本轮新交互仍待用户真机复验。
...
当前进度（2026-07-24）：settings schema v3 的 v1/v2 迁移、四形态位置恢复/回退/clamp 与仓库写盘已自动化验证；中英文核心流程已真机通过。真实退出重开后的 per-surface 恢复仍待用户验收。
...
当前进度（2026-07-24）：Electron portable 已可构建并通过核心 UI/bridge 真机验收；正式应用图标已嵌入。开机自启真机验收、installer、升级/回退和完整发布 Gate 尚未完成。
...
| 拖动/贴边/展开  | 完整         | H 切片 3 四态交互/自由位置落盘已自动验证，真机拖动/重启/多屏待验 | 完整                        |
| 托盘            | 完整         | E-F 真机通过；正式九档 ICO 已打包，视觉复验待用户                | 完整                        |
| 开机启动        | 完整         | H 切片 2 代码/打包完成（外层 portable exe），重登录待验          | Electron 原生链路           |
| 设置持久化      | 完整         | schema v3：偏好 + 四形态 placement，v1/v2 迁移/损坏恢复已测      | 含迁移                      |
...
1. 用户运行最新 Electron portable，验收正式托盘图标、开机启动勾选、重登录自启和取消后不自启。
2. 同一包分别在自动模式与固定 Orb 模式验收：边缘半隐藏 hover 后变为完整 Orb，移开 1 秒重新半隐藏；拖到左右屏幕边缘后自动吸附并在移开 1 秒后半隐藏；拖到屏幕中间松手后原位悬浮且离开不消失。点击 Orb 展开 Capsule，窗口外点击立即收起，或离开 Capsule 1 秒收起。确认自由来源回自由 Orb、边缘来源回同边半隐藏。退出重开后位置保持，再覆盖双屏内部接缝、侧边任务栏、原显示器断开与混合 DPI。
3. 上述切片 2/3 真机验收通过后，完成共享玻璃材质与全量对比度 a11y 审计。
4. 完成 Card 剩余像素验收与真实展示模式菜单。
5. 补 installer、升级/回退和发布说明，通过 Phase 8 Gate 后再讨论旧 WPF 退役。
