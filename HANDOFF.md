...
> 最后更新：2026-07-24（**时区测试失败已修 + 新增 .gitattributes + check 整体 exit 0**：① `sessionLogReaderTimezone.test.js` 第 3 个测试（UTC 对照）此前因未注入 `now` 导致 cutoff 用真实系统日期、把 fixture 桶 `2026-07-17` 滤掉而失败；根因是测试设计缺陷（同 AGENT_LESSONS L9/L26 类），**实现无 bug，未碰 §6 稳定层**，给前 3 个测试注入与 fixture 同一时刻的 `now` 修复。② 新增 `.gitattributes`（`* text=auto eol=lf` + `*.ps1 binary` 保护 BOM + 二进制资源显式 binary）+ `git add --renormalize`，从根上消除 `core.autocrlf=true` 制造的 CRLF/LF 行尾假象（§9 L37 误判的成因）。③ 本轮核实更正：§4.4 旧版"`format:check` 报 30 文件不符"是误判——HEAD 干净树上 `format:check` 本就全过，那 30 文件是 `prettier --write` 行尾假象，已还原。现 **`npm run check` 整体 exit 0**：typecheck+lint(173)+format+`npm test`(498)+build+validate+`test:server`(49) 全过，合计 **547 绿 / 0 失败**。）
> 项目：`D:\TokenUsage\plugins\codex-usage-monitor`
> 分支：`main`
> HEAD：`760cf2a`（E-F+G 已合并）。工作区有本轮未提交改动：`tests/sessionLogReaderTimezone.test.js`（修复）+ `.gitattributes`（新增）+ `HANDOFF.md`（本文档）（见 §4.4）。
...
**Milestone E-F + G 合并已完成（2026-07-24），真机手测已通过（用户确认 2026-07-24）**。范围：托盘菜单 + 设置持久化（theme/client/language/displayPreference 落盘）+ i18n 切换 UI + a11y 核心可达。已 commit `760cf2a`（注：§5/§8.1b 旧版写的 `d4e8dce` 是 amend 前的 dangling hash，origin/main 实际是 amend 后的 `760cf2a`）于 `feat/milestone-e-f-g` 分支 ff 合并至 main。**当前 `npm run check` 整体 exit 0：547 绿 / 0 失败**（`npm test` 498 + `test:server` 49；时区测试失败已于本轮修复，格式债已清零，详见 §4.1.1 + §4.4）。
...
`npm run check`：**整体 exit 0**（2026-07-24 修复后首次真正全绿）。typecheck ✓（server/desktop/tests 三 bucket）/ lint ✓（173 文件）/ **format ✓（All matched files use Prettier code style）** / **`npm test` = 498 绿 exit 0**（renderer .ts 210 / renderer 根 .tsx 10 / renderer components .tsx 174 / electron .ts 104）/ build ✓ / validate:plugin ✓ / **`test:server` = 49 绿 exit 0** / `git diff --check` exit 0。
**测试总数**：`npm test` 498 + `test:server` 49 = **547 绿 / 0 失败**。
...
1. **Milestone H**：Electron 打包（portable/installer；当前 portable.iss 打的是旧 WPF，Electron 无打包路径；需 extraResources 把 ps1 解出 asar）；开机自启（`app.setLoginItemSettings`，与打包一起做因开发模式自启难验证）；托盘图标美术资源（多分辨率 .ico）；位置/吸附边持久化（per-surface 坐标，需真机多屏验证）。
2. **共享层玻璃材质整体优化**（aurora wash 强度、内部高光、双层边缘、环境光晕）—— Orb v3 遗留的 3 项（§7 末）；含全量对比度 a11y 审计。
3. **Card 剩余小事**（§8.4）：Codex Card 像素验收、CardHeader 展示模式菜单实际功能。
- 真机验收通过后回填此处 + DEVELOPMENT-PLAN §15 Phase 6/7 Gate。
- 注：时区测试失败 + format 债已于 2026-07-24 修复（§4.1.1/§4.4），`npm run check` 现整体 exit 0。
