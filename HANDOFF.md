...
> 最后更新：2026-07-24（**E-F+G 真机手测通过 + 已合并 main**：用户确认托盘/偏好持久化/i18n/a11y 手测全过。commit `d4e8dce` 于 `feat/milestone-e-f-g` 分支已 ff 合并至 main 并 push origin。§4.4/§5/§8 状态从"待手测/待 commit"更正为"已通过/已合并"。test 脚本拆分（npm test 498 绿 + test:server 48 绿+1 既有 fail）同在此 commit。）
> 项目：`D:\TokenUsage\plugins\codex-usage-monitor`
> 分支：`main`
> HEAD：`e2210d4`（D-3 已提交推送）。工作区有 E-F+G 未提交改动。
...
**Milestone E-F + G 合并已完成（2026-07-24），真机手测已通过（用户确认 2026-07-24）**。范围：托盘菜单 + 设置持久化（theme/client/language/displayPreference 落盘）+ i18n 切换 UI + a11y 核心可达。已 commit `d4e8dce` 于 `feat/milestone-e-f-g` 分支并合并至 main。**当前 546 测试绿 + 1 既有 server 时区测试失败**（`tests/sessionLogReaderTimezone.test.js`，日期 2026-07-17 桶漂移，与 E-F 完全无关，干净树同样失败，留 server 层单独修；详见 §4.1.1）。

**架构核心：主进程为偏好单一真相源**。`SettingsRepository`（`electron/settings/repository.ts`）是唯一权威，托盘菜单和 renderer 都经它读写。
- `shared/settings.ts`：schema（Settings: version/themePreference/displayPreference/activeClient/language）+ `validateSettings`（纯函数，畸形输入返默认，五类输入覆盖）+ `normalizePreference`（setPreference IPC 校验）+ `resolveLanguageFromLocale`（locale→Language 纯函数，**放在 shared 供测试真实调用**）+ `DEFAULT_SETTINGS`。
...
- **托盘菜单**（`electron/tray/menu-builder.ts` + `tray/index.ts`）：
  - `buildTrayMenuTemplate(settings, callbacks)` 纯函数（可单测，10 测试）：打开Card / 展示模式(Auto|Card|Bar|Orb) / 客户端(Codex|ZCode) / 主题(Auto|Light|Dark) / 语言(简体中文|English) / 刷新 / 退出。当前选中项 type:"radio" + checked。菜单文案主进程内嵌中英文字典（`TRAY_STRINGS`，按 settings.language 选，不经过 react-i18next）。
  - `createTray({repo, callbacks})`：new Tray + 占位图标（nativeImage 16x16 内嵌 PNG，视觉留 Milestone H 美术资源）+ setContextMenu + `rebuild()`（preference 变化时重建菜单刷新 ✓）+ `destroy()`。
- **main.ts 接线**：启动 `settingsRepo.load()` → `applyThemePreference`（nativeTheme.themeSource 同步）→ `registerDesktopIpc(..., settingsRepo)` → `createTray`（capture 模式跳过）。`commitPreference(key,value)` 单一入口：repo.update + broadcastPreferences + tray.rebuild + 应用副作用（theme→nativeTheme / display→启停 watcher + showOnly / client→resizeCardWindow）。`applyDisplayPreference`：auto 启动 AutoSurfaceWatcher + OrbHoverController，非 auto 停 watcher + 固定 surface。shutdown 销毁 tray + flush repo。
...
**已知限制**：① 托盘图标是 16x16 占位（视觉留 Milestone H）。② 开机自启未做（留 Milestone H，与打包一起）。③ 位置/吸附边持久化未做（per-surface 坐标持久化依赖 Milestone H 真机多屏验证）。④ OrbHoverController 的 `#isReducedMotion()` 仍返 false（占位）——reduced-motion 不应跳过 dwell 延迟（dwell 是交互防抖非视觉动画，跳过会导致误触），由 globals.css 处理 CSS 动画即可，留待统一审视。⑤ 全量对比度审计未做（留玻璃材质优化）。

**D-2/D-3 状态**：全部完成并真机验收通过（详见 §5 历史 + §7 契约）。

### 4.1.1 预先存在的测试失败（非本轮引入）

`tests/sessionLogReaderTimezone.test.js` 的"同一 UTC 时刻在 UTC 时区分桶为 UTC 当日"测试失败（期望 `2026-07-17` 桶，实际空）。在 `e2210d4`（D-3 commit）干净树同样失败，属 server 稳定层的日期漂移（fixture 含固定时间戳，随系统日期推移 bucket key 失配，类似 AGENT_LESSONS L9/L26）。**未修**：server 层改动需单独评估（§6 红线），且不属 E-F 范围。留作 server 层独立任务。
...
`npm run check`：typecheck ✓（server/desktop/tests 三 bucket）/ lint ✓（173 文件）/ format ✓ / **`npm test` = 498 绿 exit 0**（renderer .ts 210 / renderer 根 .tsx 10 / renderer components .tsx 174 / electron .ts 104）/ build ✓ / validate:plugin ✓ / **末步 `test:server` = 48 绿 + 1 既有失败 exit 1**（导致 check 整体 exit 1，但前 6 步全过且报告完整可见）/ `git diff --check` exit 0。
**测试总数**：`npm test` 498 绿 + `test:server` 48 绿 = **546 绿**，外加 1 既有 server 时区失败（§4.1.1，与 E-F 无关，干净树同样失败，未修——server 稳定层独立任务）= 547 总。
**test 脚本拆分（本轮）**：原 `npm test` 用 `&&` 串联 5 bucket，server bucket 的 1 个既有失败会在第 1 步中断，后续 4 bucket（498 个 E-F 相关测试）根本不执行——§4.4 旧版"check 时 test=546 绿"的验证场景实际跑不出来。已拆分：`npm test` 只含 4 个会绿的 bucket（build:server 前置 + renderer×3 + electron×1，永远 exit 0）；新增 `test:server` 单独跑 server；`check` 把 `test:server` 放最末步，使前 6 步（含 build/validate）报告完整可见，仅在末步因 server 既有失败 exit 1。未用 `;` 串联——Windows cmd 不识别 `;` 为分隔符；拆分脚本跨平台可靠且语义更清晰。
...
- ❌ **未通过 / 延期**：开机启动（留 Milestone H）、位置/吸附边持久化（留 H 真机多屏）、托盘图标美术资源（占位）、完整 Phase 6/7 Gate 未做（多屏/DPI/设置迁移/a11y 全量审计/打包均未真机验收）。
...
- **Milestone E-F + G（偏好 / 托盘 / 持久化）**：
  ...
  - **Settings schema（`shared/settings.ts`）**：`{ version: 1, themePreference, displayPreference, activeClient, language }`。只存偏好，不存凭据/配额快照/坐标（§6 + AGENTS.md 红线）。`validateSettings` 纯函数畸形输入返默认（五类输入覆盖）；`normalizePreference` 用于 setPreference IPC 校验。路径 `app.getPath("userData")/settings.json`（Electron 惯例，Win 上即 `%APPDATA%\codex-usage-monitor\`）。
  - **SettingsRepository 容错**：load 文件缺失/损坏/校验失败 → 回退 DEFAULT_SETTINGS（不阻塞启动）；update 原子写（.tmp→rename，Windows 同卷原子）+ flush() 等写盘；值未变不产生新引用（调用方据此判别是否广播）。
  ...
  - **托盘菜单**（`electron/tray/menu-builder.ts` 纯函数 + `tray/index.ts`）：`buildTrayMenuTemplate(settings, callbacks)` 是纯函数（可单测，10 测试覆盖结构/选中✓/回调/多语言）。菜单：打开Card / 展示模式(Auto|Card|Bar|Orb) / 客户端(Codex|ZCode) / 主题(Auto|Light|Dark) / 语言(简体中文|English) / 刷新 / 退出。当前选中项 `type:"radio" + checked`。文案主进程内嵌中英文字典（`TRAY_STRINGS`，按 settings.language 选，**不经过 react-i18next**——Electron 主进程菜单文案在主进程渲染）。偏好变化时 `tray.rebuild()` 重建菜单刷新 ✓。
  - **commitPreference 单一入口**（`main.ts`）：托盘 callbacks 和（未来）setPreference IPC 都经它 = repo.update + broadcastPreferences + tray.rebuild + 应用副作用（theme→nativeTheme.themeSource / display→applyDisplayPreference 启停 watcher + showOnly / client→resizeCardWindow）。副作用不遗漏。
  - **applyDisplayPreference**：auto → 启动 AutoSurfaceWatcher + OrbHoverController（生产路径）；非 auto → 停 watcher + 固定 showOnly(目标 surface)。displayPreference 的非 auto 值就是 SurfaceKind（card/indicator-bar/orb）。
  ...
  - **托盘图标**：16x16 占位（nativeImage 内嵌 PNG）。视觉留 Milestone H 美术资源（多分辨率 .ico）。
  - **未做（留后续）**：开机自启（`app.setLoginItemSettings`，与 Milestone H 打包一起）；per-surface 位置/吸附边持久化（需真机多屏验证）；全量对比度 a11y 审计（玻璃材质优化时做）；设置弹窗 UI（用户选定仅托盘）。
...
**托盘 + 设置持久化 + i18n + a11y 全部 ✅ 完成 + 真机验收通过**（2026-07-24 用户确认）。commit `d4e8dce` 于 `feat/milestone-e-f-g` 分支已合并 main。经 4 轮验收修复。详见 §4.4 + §5 历史。
...
1. **修复预先存在的时区测试失败**（§4.1.1）：`tests/sessionLogReaderTimezone.test.js` 的 fixture 日期漂移，server 层独立任务（动 §6 稳定层需单独评估）。
2. **Milestone H**：Electron 打包（portable/installer；当前 portable.iss 打的是旧 WPF，Electron 无打包路径；需 extraResources 把 ps1 解出 asar）；开机自启（`app.setLoginItemSettings`，与打包一起做因开发模式自启难验证）；托盘图标美术资源（多分辨率 .ico）；位置/吸附边持久化（per-surface 坐标，需真机多屏验证）。
3. **共享层玻璃材质整体优化**（aurora wash 强度、内部高光、双层边缘、环境光晕）—— Orb v3 遗留的 3 项（§7 末）；含全量对比度 a11y 审计。
4. **Card 剩余小事**（§8.4）：Codex Card 像素验收、CardHeader 展示模式菜单实际功能。
- 真机验收通过后回填此处 + DEVELOPMENT-PLAN §15 Phase 6/7 Gate。
