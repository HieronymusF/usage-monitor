...
function makeTrayCallbacks(
  commit: (
    key: "themePreference" | "displayPreference" | "activeClient" | "language",
    value: string,
  ) => Settings,
): TrayMenuCallbacks {
  return {
...
// ─── 连续快速修改四项偏好 → flush/restart → 最终值完整恢复 ───

test("集成：连续快速修改四项偏好 → flush → restart → 全部最终值完整恢复", async () => {
  await withTempSettings(async (dir) => {
    const repo = new SettingsRepository({ dir });
    repo.load();
    const effects = makeFakeEffects();
    ...
    // 4 次变更都广播 + rebuild（每次值都变）。
    assert.equal(effects.broadcasts.length, 4, "4 次变更都广播");
    assert.equal(effects.rebuilds, 4, "4 次变更都 rebuild");
    // 无残留 tmp。
    for (let i = 1; i <= 10; i++) {
