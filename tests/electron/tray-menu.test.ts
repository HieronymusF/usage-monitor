...
function makeCallbacks(): TrayMenuCallbacks & { calls: string[] } {
  ...
  return {
    ...
    setAutoLaunch: (enabled) => calls.push(`autoLaunch:${enabled}`),
...
test("结构：顶层含打开Card / 4 子菜单 / 刷新 / 开机启动 / 退出", () => {
  const menu = buildTrayMenuTemplate(DEFAULT_SETTINGS, makeCallbacks());
  // 顶层项数 = 1(openCard) + 1(sep) + 4(submenu) + 1(sep) + 1(refresh) + 1(autoLaunch) + 1(quit) = 10
  ...
  assert.equal(menu[8]?.label, "开机启动");
  assert.equal(menu[8]?.type, "checkbox");
  assert.equal(menu[8]?.checked, false);
  assert.equal(menu[9]?.label, "退出");
});
...
test("click 回调：openCard / refresh / autoLaunch / quit", () => {
  ...
  (menu[9] as { click?: () => void }).click?.();
  assert.deepEqual(cb.calls, ["openCard", "refresh", "autoLaunch:true", "quit"]);
});
...
test("TRAY_STRINGS 导出两种语言", () => {
  ...
  assert.equal(TRAY_STRINGS.en.autoLaunch, "Launch at startup");
});
