 ...
 * - 拖动 > 6 DIP → moveOrb 被调用 + dragOrbEnd（保存自由位置）
 * - bounds 未就绪前不 moveOrb（竞态：getOrbBounds 异步未返回时 move 不应发生）
 * - pointercancel → 不展开、不保存（中断）
...
function installMockMonitor(): MonitorCalls {
  ...
  (globalThis as { window: typeof globalThis & { monitor?: unknown } }).window.monitor = {
    ...
    resumeHover: (dragged: boolean) => {
      calls.resumeHover.push(dragged);
    },
...
test("useOrbDrag: 正常 click → showSurface edge-capsule", async () => {
  ...
  assert.equal(calls.dragOrbEnd, 0, "click 不保存拖动位置");
  assert.deepEqual(calls.resumeHover, [false], "click 恢复为非拖动态");
  clearMockMonitor();
});

test("useOrbDrag: 拖动 > 6 DIP → moveOrb + 保存自由位置", async () => {
  const calls = installMockMonitor();
  const ref = {
    handlers: null as ReturnType<typeof useOrbDrag> | null,
    target: null as HTMLElement | null,
  ...
  assert.equal(calls.dragOrbEnd, 1, "松手保存自由位置");
  assert.deepEqual(calls.resumeHover, [true], "拖动结束进入 floating 状态");
...
test("useOrbDrag: pointercancel → 中断，不展开不保存", async () => {
  const calls = installMockMonitor();
  const ref = {
    handlers: null as ReturnType<typeof useOrbDrag> | null,
    target: null as HTMLElement | null,
  ...
  assert.equal(calls.dragOrbEnd, 0, "cancel 不保存");
  assert.equal(calls.showSurface.length, 0, "cancel 不展开");
  assert.deepEqual(calls.resumeHover, [false], "cancel 恢复原 Orb 状态");
  clearMockMonitor();
});
...
test("useOrbDrag: getOrbBounds 返回 null（无可见窗口）→ 不展开不移动", async () => {
  ...
  assert.equal(calls.showSurface.length, 0, "bounds=null 拖动不展开");
  assert.equal(
    calls.dragOrbEnd,
    1,
...
test("P1-1: pointerdown 立即 suspendHover，pointerup resumeHover", async () => {
  ...
  await new Promise((r) => setTimeout(r, 10));
  assert.deepEqual(calls.resumeHover, [false], "click pointerup 以 dragged=false 恢复");
  clearMockMonitor();
});
...
test("P1-2: down→up 超过 6 DIP 无 pointermove 仍判定为拖动（不展开）", async () => {
  ...
  assert.equal(calls.dragOrbEnd, 1, "判为拖动→保存自由位置");
  assert.deepEqual(calls.resumeHover, [true], "判为拖动→主进程按最终位置决定 revealed/floating");
...
test("P2: pointerup 最终坐标在 dragOrbEnd 之前发送（最后一次 move 到 up 仍有位移）", async () => {
  ...
  assert.equal(calls.dragOrbEnd, 1, "保存发生在最终坐标之后");
  clearMockMonitor();
});
...
test("P1-2: down→up 位移 < 6 DIP 无 pointermove 仍判为 click（展开）", async () => {
  ...
  assert.equal(calls.dragOrbEnd, 0, "判为 click→不保存拖动位置");
  clearMockMonitor();
});
